...
class Trail {
  ...
  _createGeometry(modelMatrix) {
    ...
    for (let i = 0; i < UPDATE_COUNT_OF_PARTICLE_COUNT; i++) {
      const ci =
        (this._positionIndex %
          (totalParticleCount * POSITION_ATTRIBUTE_COUNT)) +
        i * POSITION_ATTRIBUTE_COUNT;

      let subPosition = position;

      if (this._oldPosition) {
        const step = Cartesian3.multiplyByScalar(
          diff,
          i / UPDATE_COUNT_OF_PARTICLE_COUNT,
          scratchStep
        );
    ...
    for (let i = 0; i < UPDATE_COUNT_OF_PARTICLE_COUNT; i++) {
      const ci =
        (this._mouseIndex % (totalParticleCount * MOUSE_ATTRIBUTE_COUNT)) +
    ...
    this._positionIndex +=
      POSITION_ATTRIBUTE_COUNT * UPDATE_COUNT_OF_PARTICLE_COUNT;
  ...
  _createPrimitve() {
    ...
    const appearance = new Appearance({
      ...
      closed: false,
      renderState: {
        blending: BlendingState.ADDITIVE_BLEND, //使用Alpha混合功能启用混合
